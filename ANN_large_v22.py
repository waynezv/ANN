...
# Add Distance Prior
        ...
        #input = input[:,33,:,:].reshape(1600,1,64,64)
        #test_input = test_input[:,33,:,:].reshape(82,1,64,64)
        print(input.shape)
        ...
        model.add(Convolution2D(256,6,6, \
                subsample=(2,2), \
                activation='relu'))
        model.add(Convolution2D(1024,32,32, \
                subsample=(1,1), \
                activation='relu'))
        ...
        # Compile
        sgd = SGD(lr=0.1, decay=0.009, momentum=0.9, nesterov=False)
